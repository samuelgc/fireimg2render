...
class ParamRenderFeedback:
    ...
    def __init__(self):
        ...
        dense0 = tf.layers.dense(flat_plus, units=1024, activation=tf.nn.relu, name='dense0')
        dense1 = tf.layers.dense(dense0, units=512, activation=tf.nn.relu, name='dense1')
        self.encoded = tf.layers.dense(dense1, units=10, activation=tf.nn.relu, name='encoded')

        #Decoding
        decode2 = tf.layers.dense(self.encoded, units=512, activation=tf.nn.relu, name='decode2')
        decode1 = tf.layers.dense(decode2, units=1024, activation=tf.nn.relu, name='decode1')
        ...
        tf.summary.image("predicted", self.decoded)
        tf.summary.scalar("param loss", self.param_loss)
        tf.summary.scalar("image loss", self.image_loss)
        self.merge = tf.summary.merge_all()

    def phase_1(self, fresh=False, norm=True, sample_size=400, batch_size=10):
        if fresh:
        ...
        while change_count < 3:
            ...
            for count in range(len(data)):
                try:
                    ...
                    target_in = [params]
                    feed_dict = {self.input: input_in, self.intrinsic: intrin_in, self.target: target_in, self.render: input_in}
        ...
        while change_count < 3:
            ...
            for count in range(len(data)):
                try:
                    ...
                    img = Image.open(data[item])
                    img = img.resize((128, 128), Image.BICUBIC)
                    # img.thumbnail((128, 128), Image.ANTIALIAS)
                    img_in = np.asarray(img)
                    img_in = img_in / 255.0
                    input_in = [img_in]
                    intrin_in = [getIntrinsics("./ifds/fire_{}.ifd".format(selected))]
                    feed_dict = {self.input: input_in, self.intrinsic: intrin_in}
                    output, _ = self.sess.run([self.encoded, self.decoded], feed_dict=feed_dict)

                    encoded_in = [output[0].copy()]
                    encoding = denormalize(output[0])
                    # print "Params: {} {} {} {} {} {} {} {} {} {}".format(encoding[0], encoding[1], encoding[2], encoding[3], encoding[4], encoding[5], encoding[6], encoding[7], encoding[8], encoding[9])
                    ...
                    render_in = [rendered]
                    feed_dict = {self.input: input_in, self.target: encoded_in, self.intrinsic: intrin_in, self.render: render_in}
                    summary, loss, _ = self.sess.run([self.merge, self.image_loss, self.image_train], feed_dict=feed_dict)
                    summary_write.add_summary(summary, x)
                    x += 1
                    total_loss += loss
                except Exception as fail:
                    print "Got a problem:", fail
                    continue
            epoch += 1
            mse = total_loss / len(data)
            if abs(mse - last_mse) < 0.0005:
                ...
                change_count = 0
            last_mse = mse
            print "Phase 2--Epoch: {} --> Average Loss: {}".format(epoch, total_loss / len(data))
    ...
    def test(self, filename, attempt):
        ...
        output, _ = self.sess.run([self.encoded, self.input], feed_dict=feed_dict)
